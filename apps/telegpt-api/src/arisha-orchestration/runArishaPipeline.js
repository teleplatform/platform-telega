...
export async function runArishaPipeline(req) {
    const state = createInitialState(req.trace_id, req.session_id, req.actor_id, req.actor_role, req.channel, req.raw_input);
    ...
    if (deliveryPolicy.voice_enabled) {
        state.voice_request = {
            ...
            language: "ru",
            policy_scope: req.actor_role === "creator" ? "voice.creator" : "voice.public",
            fallback_allowed: true,
            render_strategy: "final_render",
            trace_id: req.trace_id,
        };
        try {
            const voiceResult = await routeVoiceRequest(state.voice_request, req.actor_role);
            state.surface_reply = {
                channel: req.channel,
            ...
            const message = error instanceof Error ? error.message : "Unknown voice routing error";
            state.surface_reply = {
                channel: req.channel,
...
//# sourceMappingURL=runArishaPipeline.js.map