 ...
 */
async function tryKozy(text, traceId) {
    ...
    try {
        ...
        const audioFilePath = result.file ?? "";
        const audioFilename = result.filename ?? filename;
        const audioSizeBytes = result.size_bytes ?? 0;
        if (!audioFilename) {
            return null;
        }
        return {
 ...
 */
async function tryCosyVoiceStub(_text) {
...
export async function routeVoiceRequest(req, _actorRole) {
    const t0 = Date.now();
    // Attempt 1: Kozy local XTTS (primary)
    const kozyResult = await tryKozy(req.text, req.trace_id);
    if (kozyResult) {
        const latencyMs = Date.now() - t0;
        console.log(`[voiceRouter] Kozy succeeded: ${kozyResult.audio_filename} (${kozyResult.audio_size_bytes}b) in ${latencyMs}ms`);
        return {
    ...
    if (cosyResult) {
        const latencyMs = Date.now() - t0;
        console.log(`[voiceRouter] CosyVoice stub succeeded: ${cosyResult.audio_filename} in ${latencyMs}ms`);
        return {
            text_output: req.text,
    ...
    const latencyMs = Date.now() - t0;
    console.warn(`[voiceRouter] All voice providers failed for trace_id=${req.trace_id} in ${latencyMs}ms`);
    return {
        text_output: req.text,
