...
function buildFallbackText(input) {
    if (input.relationship_plan.relationship_state === "sensitive_support_mode") {
        return normalizeFinalText(input.presence_plan.chunks[0]?.text ??
            `${input.presence_plan.micro_ack} Можно спокойно пойти шаг за шагом.`);
    }
    if (input.composed_response.structure === "single_line_ack") {
        return normalizeFinalText(`${input.presence_plan.micro_ack} Уточни, что именно нужно раскрыть первым.`);
    }
    if (input.composed_response.structure === "next_step_forward") {
        return normalizeFinalText(`${input.presence_plan.micro_ack} Следующий правильный шаг — держаться главной линии: ${input.attention_plan.primary_focus}.`);
    }
    return normalizeFinalText(`${input.presence_plan.micro_ack} Суть сейчас в этом: ${input.attention_plan.primary_focus}.`);
}
...
//# sourceMappingURL=runQualityGate.js.map