...
export function formatRecentEvidenceList(items) {
    if (items.length === 0) {
        return "📭 Нет недавних вызовов.";
    }
    const limited = items.slice(0, MAX_LIST_ITEMS);
    const lines = ["🛠 Sigma Forge — Recent", ""];
...
export function formatFailedEvidenceList(items) {
    if (items.length === 0) {
        return "✅ Нет Failed вызовов.";
    }
    const limited = items.slice(0, MAX_LIST_ITEMS);
    const lines = ["❌ Sigma Forge — Failed", ""];
    for (const item of limited) {
        const time = formatTime(item.startedAt);
...
export function formatBlockedEvidenceList(items) {
    if (items.length === 0) {
        return "✅ Нет Blocked вызовов.";
    }
    const limited = items.slice(0, MAX_LIST_ITEMS);
    const lines = ["⛔ Sigma Forge — Blocked", ""];
    for (const item of limited) {
...
export function formatEvidenceDetail(view) {
...
export function formatEvidenceStats(view) {
    const lines = [
        "📊 Sigma Forge Stats",
        "",
        `Total: ${view.total}`,
...
export function formatEmptyEvidence() {
    return "📭 Нет данных для отображения.";
}
export function formatEvidenceNotFound() {
