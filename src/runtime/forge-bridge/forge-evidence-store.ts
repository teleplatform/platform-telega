...
export function initEvidenceStore(config: ForgeEvidenceStoreConfig): void {
  storeConfig = config;
  currentFilePath = getStoreFilePath();
  console.log(`[evidence-store] initialized: ${currentFilePath}`);
}

export function getEvidenceStoreConfig(): ForgeEvidenceStoreConfig {
  if (!storeConfig) {
    storeConfig = { storeDir: DEFAULT_STORE_DIR };
  }
  return storeConfig;
...
export async function appendEvidenceToStore(
...
export async function updateEvidenceInStore(
  invocationId: string,
  ...
  const updated: ForgeInvocationEvidenceRecord = {
    ...current,
    ...patch,
  };
...
export async function listEvidenceFromStore(
  ...
  const records: ForgeInvocationEvidenceRecord[] = [];
  const content = readFileSync(filePath, { encoding: "utf8" });
  const lines = content.split("\n").filter((line) => line.trim());

  for (const line of lines) {
    try {
      const record = JSON.parse(line) as ForgeInvocationEvidenceRecord;
...
function applyOrdering(
  records: ForgeInvocationEvidenceRecord[],
  query?: ForgeEvidenceQuery,
...
export async function syncMemoryToStore(): Promise<number> {
