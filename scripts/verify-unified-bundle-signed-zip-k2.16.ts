...
function main() {
  ...
  if (!filePath) {
    console.error("Usage: node --import tsx scripts/verify-unified-bundle-signed-zip-k2.16.ts <path/to/unified-export-bundle.signed.zip>");
    process.exit(2);
  }
  const abs = path.resolve(process.cwd(), filePath);
  if (!fs.existsSync(abs)) {
    console.error(`[K2.16] File not found: ${abs}`);
    process.exit(2);
  }
  ...
  if (
    ...
    fail("bundle.sig.v3.json invalid");
  }
  ...
  console.log("✅ K2.16 Signed ZIP verified with exported_at-bound signature (offline)");
  ...
  console.log(`- exported_at: ${exportedAt}`);
